<?php
include_once $_SERVER['DOCUMENT_ROOT'] . '/assets/snippets/vebinar-class/vebinar.class.php';
$vebinar = new Webinar(array('method' => 'init'));
...
$fetch = $vebinar->get_all($id);

$vebinars = array();
foreach ($fetch as $row) {
    if (strtotime($row['date']) > time())
        $vebinars['upcoming'][] = $row;
    else{
        if($_GET['tag'] && $_GET['tag'] != $row['tag'])
            continue;
        $vebinars['passed'][] = $row;
    }
    ...
    <div class="vebinars-grids">
        <?php foreach (array_reverse($vebinars['upcoming']) as $row): ?>
            <div class="vebinar-grid-item upcoming"
                ...
                <button type="button" class="select-vebinar veninar-details" data-id="<?= $row['id'] ?>">Подробнее
                </button>
